...
def create_output(direc, startingMols, newTestingScoresListFinal, newCombinedScoresList, actualFrequencies, allIndexTest, allReports, delMols=None):
    '''
    ...
    Parameters:
        ...
        startingMols: List of starting molecules
        newTestingScoresListFinal: Final testing scores list
        newCombinedScoresList: Combined scores list
        actualFrequencies: List of actual frequencies
        allIndexTest: All index test results
        allReports: All reports
        delMols: List of molecules removed during fitting (optional)
    ...
    for i in range(len(newTestingScoresListFinal)):
        ...
        for n in range(len(indivTest)):
            ...
            if len(indivReport[n]) == 0:
                # Check if molecule was removed during fitting
                if delMols is not None and str(indivTest[n][2]) in delMols:
                    f.write('Warning: Molecule removed during fitting.\n')
                else:
