...
def create_dataset_file(spectrum_freqs,spectrum_ints, ll0,ul0, localYN, localDirec, direc, edges, smiles, allVectors, countDict, vectorSmiles, maxMols, temp, dishSize, sourceSize, cont, data, resolution, freq_arr, dv_val_freq, dv_val_vel, dfNames, dfSmiles, dfIso):
    ...
    if localYN == True:
        ...
        for filename in os.listdir(localDirec):
            ...
            if os.path.isfile(q) and q.endswith(
                ...
                minFreq = ll0
                maxFreq = ul0
                src = molsim.classes.Source(Tex=temp, column=1.E10, dV = dv_val_vel,continuum = cont)
                sim = molsim.classes.Simulation(mol=mol, ll=minFreq, ul=maxFreq, observation = data, source=src, line_profile='Gaussian', use_obs=True)
                if len(sim.spectrum.freq_profile) > 0:
                    #find peaks in simulated spectrum.
                    peak_indicesIndiv = molsim.utils.find_peaks(sim.spectrum.freq_profile, sim.spectrum.int_profile, res=resolution, min_sep=max(resolution * ckm / np.amax(freq_arr),0.5*dv_val_freq), is_sim=True)
...
def full_dataset_creation(specPath, direc, sig, localYN, localDirec,temp, dfLocal):
    """
